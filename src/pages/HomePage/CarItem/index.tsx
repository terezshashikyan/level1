...
const CarItem: FC<ICarItemComponent> = ({car}) => {
    return (
        <div className="carItem" key={car.id}>
            ...
            {car.soldOut? <img className = 'carItem__soldout' src={SoldOut} alt="sold out" />: null}
            {car.soldOut? <span className = 'carItem__priceTag'>{car.price}</span>:null}
            </div>
            <div className="carItem__content">
            <p className='carItem__content_name'>{car.name}</p>
            <a className='carItem__content_viewDetailsLink' href="#details">View Item</a>
            </div>
        </div>
    )
}
